...
from megatron.core.optimizer.layer_wise_optimizer import (
    FlatShardedAdamFallback,
    LayerWiseDistributedOptimizer,
    build_param_range_map_from_world_range,
    build_variable_flat_shard_ranges,
)
...
def test_layer_wise_prunes_state_for_params_removed_from_groups():
    """torch.optim cannot serialize tensor state keys absent from param groups."""
    kept_param = nn.Parameter(torch.ones(2, 2))
    removed_param = nn.Parameter(torch.zeros(2, 2))
    optimizer = torch.optim.SGD([kept_param], lr=0.1)
    optimizer.state[kept_param]["momentum_buffer"] = torch.ones_like(kept_param)
    optimizer.state[removed_param]["eager_state"] = torch.ones_like(removed_param)

    with pytest.raises(KeyError):
        optimizer.state_dict()

    LayerWiseDistributedOptimizer._prune_optimizer_state_to_local_params(optimizer)

    state_dict = optimizer.state_dict()
    assert len(state_dict["state"]) == 1
    assert len(optimizer.state) == 1
    assert kept_param in optimizer.state
    assert removed_param not in optimizer.state


def test_variable_flat_shard_ranges_are_aligned_and_weighted():
    ranges = build_variable_flat_shard_ranges(1024, [0, 10, 30, 60], 128)

    assert [(r.start, r.stop) for r in ranges] == [
        (0, 0),
        (0, 128),
        (128, 384),
        (384, 1024),
...
def test_param_range_map_from_variable_world_range_can_split_tensors():
    param_a = torch.empty(100)
    param_b = torch.empty(300)
    param_index_map = {param_a: (0, 100, 0), param_b: (128, 428, 0)}

    range_map = build_param_range_map_from_world_range(param_index_map, range(64, 256))

    assert range_map[param_a]["param"] == range(64, 100)
    assert range_map[param_b]["param"] == range(0, 128)


def test_flat_adam_bucket_slice_map_can_split_tensor():
    fallback = object.__new__(FlatShardedAdamFallback)
    param_a = torch.nn.Parameter(torch.empty(10))
    param_b = torch.nn.Parameter(torch.empty(20))
    fallback._rank_ranges = [range(0, 12), range(12, 30)]
    fallback._param_index_map = {param_a: (0, 10, 0), param_b: (10, 30, 0)}

    slices = fallback.flat_param_slices_for_bucket([param_b], rank=1)

    assert len(slices) == 1
    assert slices[0]["param"] is param_b
    assert slices[0]["param_range"] == range(2, 20)
    assert slices[0]["numel"] == 18


def test_layerwise_bucket_payload_copies_flat_slices():
    whole_param = torch.nn.Parameter(torch.tensor([1.0, 2.0]))
    flat_param = torch.nn.Parameter(torch.tensor([10.0, 11.0, 12.0, 13.0, 14.0]))
    bucket = _ParamAndGradBucket(
        params=[whole_param, flat_param],
        param_data=None,
        grad_data=torch.empty(7),
        offset=0,
        numel_unpadded=7,
        gradient_scaling_factor=1.0,
        bucket_id=0,
    )
    bucket.set_layerwise_params_list(
        [[whole_param], []],
        [
            [],
            [{"param": flat_param, "param_range": range(2, 5), "numel": 3}],
        ],
    )

    rank0_payload = bucket.build_layerwise_param_sync_payload(
        rank=0, dtype=torch.float32, device=torch.device("cpu")
    )
    assert torch.equal(rank0_payload, torch.tensor([1.0, 2.0]))
    assert bucket.layerwise_param_flat_sizes == [2, 3]

    bucket.copy_layerwise_param_sync_payload(rank=1, tensor=torch.tensor([20.0, 21.0, 22.0]))

    assert torch.equal(flat_param.detach(), torch.tensor([10.0, 11.0, 20.0, 21.0, 22.0]))
...
class TestLayerWiseOptimizer:
    ...
    def create_model_and_optimizer_with_overlap_param_gather(
        ...
        bucket_size=None,
        layerwise_optimizer_memory_balance=False,
    ):
        ...
        optimizer_config = OptimizerConfig(
            ...
            layerwise_optimizer_memory_balance=layerwise_optimizer_memory_balance,
        )
    ...
    def test_muon_hyperball_overlap_preserves_radius(self):
        """MuonHyperball preserves matrix radii through memory-balanced async all-gather."""
        model = SimpleModel().bfloat16().cuda()
        model.requires_grad_(True)
        ddp_config = DistributedDataParallelConfig(
            use_distributed_optimizer=False,
        ...
        model.broadcast_params()
        initial_norms = {
            name: param.float().norm().detach().clone()
            for name, param in model.named_parameters()
            if param.ndim == 2
        }
        ...
        config = OptimizerConfig(
            ...
            bf16=True,
            use_layer_wise_distributed_optimizer=True,
            layerwise_optimizer_memory_balance=True,
            overlap_param_gather=True,
            muon_tp_mode='duplicated',
        )
        optimizer = get_megatron_optimizer(config, [model], pg_collection=pg_collection)
        assert optimizer.async_allgather
        ...
        for name, param in model.named_parameters():
            if param.ndim == 2:
                torch.testing.assert_close(
                    param.float().norm(), initial_norms[name], rtol=2e-3, atol=2e-3
                )
            replicas = [torch.empty_like(param) for _ in range(get_pg_size(pg_collection.dp_cp))]
            torch.distributed.all_gather(replicas, param, group=pg_collection.dp_cp)
            for replica in replicas[1:]:
                torch.testing.assert_close(replica, replicas[0])

    def test_memory_balanced_flat_adam_overlap_vs_sync_allgather(self):
        """Memory-balanced flat Adam slices should sync through bucket param gather."""
        overlap_model, overlap_optimizer, pg_collection = (
            self.create_model_and_optimizer_with_overlap_param_gather(
                async_allgather=True,
                layerwise_optimizer_memory_balance=True,
        ...
        for op, sp in zip(overlap_model.parameters(), sync_model.parameters()):
            torch.testing.assert_close(
                ...
                rtol=0,
                atol=0,
                msg="Memory-balanced overlap and sync paths produced different parameter updates",
            )
